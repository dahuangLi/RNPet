...
export default class DatePicker extends React.Component {
    ...
    constructor(props) {
        ...
        this.getNewDate = (values, index) => {
            ...
            if (mode === DATETIME || mode === DATEHOUR || mode === DATE || mode === YEAR || mode === MONTH) {
                switch (index) {
                    case 0:
    ...
    getTimeData(date) {
        ...
        if (mode === DATETIME || mode === DATEHOUR) {
    ...
    getValueCols() {
        ...
        if (mode === DATETIME || mode === DATEHOUR || mode === DATE) {
            cols = this.getDateData();
            value = [date.getFullYear() + '', date.getMonth() + '', date.getDate() + ''];
        }
        if (mode === DATETIME || mode === DATEHOUR || mode === TIME) {
            const time = this.getTimeData(date);
            if (mode === DATEHOUR){
                time.cols.splice(1, 1);
            }
            cols = cols.concat(time.cols);
            const hour = date.getHours();
            let dtValue = mode === DATEHOUR ? [hour + ''] : [hour + '', time.selMinute + ''];
            let nhour = hour;
            if (use12Hours) {
                nhour = hour === 0 ? 12 : (hour > 12 ? hour - 12 : hour);
                dtValue = mode === DATEHOUR ? [nhour + '', (hour >= 12 ? 1 : 0) + ''] : [nhour + '', time.selMinute + '', (hour >= 12 ? 1 : 0) + ''];
            }
    ...
    render() {
        ...
        const { disabled, pickerPrefixCls, prefixCls, rootNativeProps, className, style, itemStyle = {fontSize: 18, fontWeight: '300'} } = this.props;
